 ...
 * @name: oled_draw_line
 * @msg: 画竖线
 * @param x:横坐标,y:纵坐标,high:线高度
 * @return {*}
 */
void oled_draw_line(u8 x, u8 y, u8 high)
{
	u8 i;
	u8 oled_num = 0xff;
	u8 shangshu = 0;
	...
	yushu = high % 8;
	OLED_Set_Pos(x, y);
	for (i = 0; i < shangshu; i++)
	{
		// oled_num = oled_num>>(8-high);
		OLED_Set_Pos(x, y + i);
		OLED_WR_Byte(0xff, OLED_DATA);
	}
	OLED_Set_Pos(x, y + i);
	oled_num = 0xff >> (8 - yushu);
	OLED_WR_Byte(oled_num, OLED_DATA);
}
